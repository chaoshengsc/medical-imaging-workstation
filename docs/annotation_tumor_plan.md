...
**当前状态（2026-09-23）：** 工作站工程里程碑 A/B/C/D 已在 [AGENT_SYNC.md](AGENT_SYNC.md) 记录为通过；肿瘤功能仍未达到自动分割或逐病灶分类准入。KCL VS_Seg 作者 test split 的 46 例 T1 与参考 RTSTRUCT 已下载并配对。首轮临时 mask 转换后来发现行列轴转置错误，已隔离为不可用；按正确轴顺序重建 46 份 mask，并用独立 VTK 逐例核对空间位置（几何交叉实现 Dice 中位数 0.9654、范围 0.9276–0.9828，差异来自不同边界栅格规则）。这仍不是 SlicerRT 官方转换或模型效能证据。固定网络与 best checkpoint 已通过 `weights_only=True` / `strict=True` 全量加载（256 项、无 missing/unexpected）；裸 MONAI 0.4.0 import 因 NumPy 1.26 缺少 `np.bool`，探针仅在进程内加兼容别名。随后用全尺寸确定性合成 patch `[1,1,384,384,64]` 完成一次 CPU 前向，输出 `[1,2,384,384,64]`、65.88 秒、0.5 秒采样进程组峰值 RSS 11.73 GiB（12 GiB 终止线，余量很小且采样可能低估瞬时峰值）。这证明单 patch 形状执行可行，不证明完整整例推理预算。产品侧图像 adapter 和隔离 worker 的加载/滑窗/输出协议现已实现并通过合成契约测试；本轮没有执行 KCL 网络前向、没有读取患者影像或计算 Dice/HD95。`weights` 为 VERIFIED，`input_contract` 仅对已实现的 RAS/归一化/源空间往返工程合同及恒等 MR rescale 子集 VERIFIED，非恒等 rescale 明确拒绝；`cpu_budget`、`task_compatibility` 仍待验证，产品准入仍关闭。MONAI 0.4.0 属于官方 GHSA-x6ww-pf9m-m73m 公告覆盖版本；隔离 worker 只读固定网络组件，未调用 bundle 下载/解压 API。原始数据与机器报告位于被忽略的 Annotation_Projects/ 子目录，不进入 Git。本文是唯一计划与候选研究记录，不代替已实施架构、既有实验报告或软著材料。
...
### NeuroVFM 权重下载与哈希核对（2026-09-22，用户明确指示"下载"后）

用户明确指示"下载"。用 `huggingface_hub.hf_hub_download` 分别取回两个固定 revision 的 `pytorch_model.bin`；只下载+计算 SHA256+读取纯文本 `README.md`，未安装 torch 之外的新依赖，未执行 `torch.load`、未构造模型、未做任何前向推理。

- **`mlinslab/neurovfm-encoder`@`d5194fc70a162185f8ef062e362bd522a35312a9`**：`pytorch_model.bin` 实际 286,576,042 字节，与 2026-09-08 记录的字节数完全一致；本次首次算出 SHA256 = `744cf058edf34be85118dd2b5410da6fa50dc0ebad648c73ead86eceb1398fc7`（此前只有 revision 摘要，没有文件哈希）。下载耗时 88.96 秒。
- **`mlinslab/neurovfm-dx-mri`@`628b661744482e528374d9c9ef1b54aded3d4c6e`**：`pytorch_model.bin` 实际 3,789,726 字节，同样与 2026-09-08 记录一致；SHA256 = `e3db6eee69db0d7ef291c1a9ed6de5a9ab4dd417d3fc10b184b43c012329492b`。下载耗时 4.93 秒。
- **权重许可确认，且是关键限制**：两个仓库的 `README.md` 均写明"Weights: [CC-BY-NC-SA 4.0]…(Non-Commercial Research Use)；Code: MIT License"。这不是本轮之前"未核实"的猜测，是官方 model card 原文。**CC-BY-NC-SA 的 Non-Commercial 条款与本产品 `pyproject.toml` 声明的 `Proprietary — All rights reserved` 商业属性直接冲突**——如果 MUI 要作为商业/专有软件分发，这份权重不能未经官方另行授权就随产品打包或用于商业场景；这与此前对 CLIP-Driven（CC BY-NC-ND）的许可顾虑是同一类问题，不是新发现的模式，但这次是本项目目前唯一已下载、已算出真实哈希的权重，许可冲突从"理论顾虑"变成了"已确认会挡住产品接入的具体障碍"。
- **模型出处升级为同行评审期刊**：README 指向 [Nature Medicine 论文](https://www.nature.com/articles/s41591-026-04497-1)（此前只查过 GitHub/HF 页面），训练数据为密歇根大学健康系统 218,882 例 MRI（跨 20 年常规临床），编码器另外用 566,915 个 CT/MRI 检查做自监督预训练；诊断头架构确认为"modified AB-MIL (classify-then-aggregate)"，74 项专家定义诊断本体。训练算力为 8×L40S GPU、不到 1000 GPU-小时——供参考，不代表本机复现成本。
- **仍未做的事**：未读取论文全文核实是否有跨机构患者级独立验证（README 只说明训练机构，同 Prima 一样是同机构队列）；未验证权重内部结构与 2026-09-08 CPU 适配审计假设的键名是否完全对应；未安装除下载所需之外的任何依赖；未构造模型对象；未做任何 CPU/GPU 推理；未把这两个文件的路径接入产品代码或 `tumor_model_admission.py` 的任何证据卡。
...
这项复查只读本地准入卡与固定上游公开资料，**当时没有下载或运行新的分割模型**。现有 `tumor_model_admission.py` 固定上游提交 `33410a2`、T1 权重名 `UNet2d5_Att_Hard_T1_final.zip` 与 Zenodo DOI。作者 [GitHub 仓库](https://github.com/KCL-BMEIS/VS_Seg) 和 [Zenodo 产物](https://zenodo.org/records/6323472) 仍公开列出该约 34.4 MB 的训练后权重包；截至该次只读复查时，本机尚无权重包，因此当时的本地 SHA、内部文件、CPU 严格加载与 mask 输出均未验证。权重后来按用户授权取得，见下方“同日 T1 权重包下载与完整性核验”；公开元数据本身不等于模型准入通过。
...
**当前状态：** 权重包身份与固定上游网络 `strict=True` 全量加载已验证（256 项，missing/unexpected 为空）。隔离 venv 使用 MONAI 0.4.0、Torch 2.5.1、NumPy 1.26.4；探针仍需进程内 `np.bool = bool` 兼容别名。全尺寸单 patch 合成 CPU 前向输出正确且有限（65.88 秒，11.73 GiB 采样峰值），但余量很小，不能据此确认整例 CPU 预算。新增无标签适配器已在 5 种合成方向/斜采集上与 MONAI 0.4 Orientation+Normalize 数组和 affine 逐项一致，合成 mask 可精确回到来源 z-y-x 网格。另对作者 test split 的 5680 张 MR DICOM 仅读取头信息，46 个 T1 序列的 RescaleSlope/Intercept 均为恒等变换；适配器只接受该恒等 rescale 子集，非恒等或无效值 fail-closed。因此 `input_contract` 是受限工程合同的 VERIFIED，不代表任意 MR 输入或完整 Slicer NIfTI 互通验证。未读取像素、未执行患者前向或效能评估；`cpu_budget`、`task_compatibility`、患者/阴性/OOD 验证及自动执行仍未通过，产品执行门继续关闭。压缩包日志/示例图不算本项目独立评估证据。VS-SEG-002/003 属作者 training split，只用于工程调试；作者 test 影像与 RTSTRUCT 已下载但尚未用于模型效果评估。
...
- Apple Silicon 主机上的 `/Applications/Slicer.app` 与 SlicerRT 扩展为 `amd64` 构建，不能在 arm64 原生运行；本轮未安装 Rosetta、未改系统设置，也没有冒称已走官方 SlicerRT 转换。为检查参考 mask 的像素空间，使用本机预装的 `rt-utils 1.2.7` 与 VTK 9.6.0 做只读探索。首轮转换虽通过尺寸、spacing、origin、direction 和切片引用核对，但错误地将 rt-utils 数组按 `(2,1,0)` 转为 NIfTI；独立 VTK 逐点转换揭示其 mask 在面内被转置。46 份首轮文件现保留在 `rtutils-reference-masks-axis-transposed-invalid/` 并标为 `INVALID_SPATIAL_AXIS_ORDER_SUPERSEDED`，禁止用于评估。
- 修正为 `(2,0,1)` 后重建 46 份 NIfTI，保存到 `rtutils-reference-masks-axis-corrected/`。所有 46 例的文件空间属性与 T1 一致、非空，目标占用切片均等于 RTSTRUCT contour image 引用切片，逐例 SHA-256 已记录。另用 VTK `vtkPolyDataToImageStencil` 将 DICOM LPS 轮廓物理坐标映射到 SimpleITK 图像索引，并按切片栅格化；与修正后的 rt-utils 输出在 46/46 病例中对齐到相同切片，几何交叉实现 Dice 中位数为 `0.9654278`，范围 `0.9275848–0.9827507`。此数值只衡量两种轮廓填充/边界栅格实现的一致性，不是模型 Dice、HD95 或临床标注质量。机器可读结果为 `axis-corrected-vtk-crosscheck.json`；旧首轮汇总 `rtutils-conversion-summary.json` 已标记为无效并记录原因。
- 修正后结果状态为 **`AXIS_CORRECTED_CROSSCHECK_COMPLETE`**：这是有独立 VTK 空间定位核对的临时参考 mask，不等于 SlicerRT 官方转换逐体素复现，也不证明边界填充完全一致。它们来自作者 test 划分，可用于冻结流程下的探索性患者级验证。checkpoint 严格加载已通过；无标签输入和源空间往返、模型 CPU 资源与患者级验证执行关口仍未完成，因此不跑患者推理、不报告效能，产品准入仍关闭。

**2026-09-23 静态依赖与资源评估：** 固定上游 requirements.txt 锁定 torch~=1.6.0、MONAI==0.4.0、torchvision~=0.7.0、numpy~=1.19.0 等。实测本机 arm64 boa 为 Python 3.10.20、torch 2.5.1、NumPy 1.26.4、torchvision 0.20.1、pydicom 2.4.5、nibabel 5.4.2、matplotlib 3.10.8；MONAI、natsort、TensorBoard 未安装。只有 MONAI 缺失会阻挡按固定源码直接构造网络；可绕开完整 VSparams.py，后者另需 natsort/TensorBoard。作者网络实际用 PyTorch 原生模块，并依赖 MONAI 的 Norm/Act factory、SkipConnection、same_padding 和旧的装饰器导入路径。PyPI 上 MONAI 0.4.0 是纯 Python wheel，350,941 bytes，最低元数据要求 Python>=3.6、torch>=1.5、NumPy>=1.17，现有 boa 满足这些声明，但旧 MONAI 与 torch 2.5.1 的运行兼容性没有验证。作者锁定的 torch 1.6 早于官方 Apple Silicon 原生 PyTorch 包（1.12 prototype），因此不能把原 requirements 当作可直接安装的 arm64 环境。预装 denoise 有 MONAI 1.5.0，但 torch 导入因重复 libomp.dylib abort；且 1.5.0 已移除作者导入的 monai.utils.aliases 旧路径，不能直接替换。机器为 10 CPU 核、32 GiB 统一内存、可用磁盘约 26 GiB；checkpoint 单个 best 文件 13.94 MB，作者滑窗 ROI 为 384×384×64，单个 FP32 输入约 36 MiB、首层 16 通道全分辨率特征图约 576 MiB；整次推理峰值 RSS 和 CPU 时间不能由这些下界推断。静态评估完成时未建环境、安装依赖或加载权重。

**依赖 wheel 下载（2026-09-23）：** 用户允许下载后，从官方 PyPI 取得 `monai-0.4.0-202012151415-py3-none-any.whl`，保存于 Git 忽略目录 `Annotation_Projects/vs-seg-t1-20260923/dependency-source/`；大小 350,941 bytes，SHA-256 `da0395de904acdfeb261dbbe46d6fecadfc274991385604dcf5e5b49a483242e` 与官方 PyPI 元数据一致。下载后初检只检查 ZIP 成员名称（137 项，无绝对路径或 `..`），当时未解包或安装；后续隔离探针环境的使用见下一节。MONAI 官方安全公告 [GHSA-x6ww-pf9m-m73m](https://github.com/Project-MONAI/MONAI/security/advisories/GHSA-x6ww-pf9m-m73m) 覆盖 <=1.5.0 的 bundle ZIP 路径穿越漏洞；本次获准的严格加载探针只导入网络组件，没有调用 bundle 下载/解压功能或处理外部压缩包。下载与哈希只证明依赖文件身份；严格加载结果见下一节。

**2026-09-23 VS-Seg strict-load 探针：** 在忽略目录 `Annotation_Projects/vs-seg-t1-20260923/cpu-strict-load-20260923/` 创建一次性 Python 3.10.20 venv（共享 boa 的只读 site-packages），用 `pip --no-index --no-deps` 安装已校验 MONAI 0.4.0 wheel。固定 KCL commit `33410a2d44e3f57b4df1c3ed005e6d40c0824aa6` 的三个必要网络源码文件，逐个通过 Git blob SHA 核对；模型参数直接照固定 `VSparams.py`，未导入其训练/推理副作用。由于裸导入 MONAI 0.4.0 在 NumPy 1.26.4 下因 `np.bool` 缺失报错，探针进程临时设置等价的 `np.bool = bool` 后导入成功，不修改依赖源码或产品环境。重新核对外层权重 ZIP 与 best checkpoint SHA，再通过 Python ZIP 流读取 checkpoint，`torch.load(..., weights_only=True, map_location='cpu')` 得到 256 个张量（3,455,790 个元素）；固定网络 `load_state_dict(..., strict=True)` 成功，missing/unexpected keys 均为空。运行时 arm64 / CPU 单线程 / Python 3.10.20 / Torch 2.5.1 / MONAI 0.4.0 / NumPy 1.26.4。报告 `strict-load-result.json` 记录 `forward_calls=0`、`patient_input_read=false`；未执行合成或患者前向。报告与源码清单已由 `.gitignore` 忽略。该结果只把准入卡 `weights` 标为 VERIFIED，不证明直接 import 兼容、CPU 推理可用性、输入/空间合同或模型效能。

**2026-09-23 VS-Seg 合成 CPU shape 冒烟：** 使用固定 KCL `VSparams.py` 参数和同一权重，在一次性 venv 中以单线程 CPU 对确定性合成输入 `[1,1,384,384,64]` 做一次模型前向，返回 logits `[1,2,384,384,64]` 且所有 logits 有限，模型调用耗时 65.88 秒。独立 supervisor 每 0.5 秒按进程组采样 RSS，峰值观测 11.73 GiB，未触发 12 GiB 终止线，900 秒超时也未触发；监控并非硬内存上限，可能低估采样间瞬时峰值。运行无患者输入、无 mask 指标计算，仅证明当前 CPU 环境能处理一个作者 ROI 尺寸的合成 tensor。首轮 worker 在成功返回后把 `SystemExit(0)` 当异常打印了误导性附加行，报告记录该情况；之后修正错误捕获逻辑，未为该日志问题重复约 12 GiB 峰值的前向。机器报告与脚本在忽略目录 `Annotation_Projects/vs-seg-t1-20260923/cpu-strict-load-20260923/synthetic-forward-result.json` 及同目录 `.py` 文件。该初步结果不解除 `cpu_budget` 准入门。

**2026-09-23 VS-Seg 产品侧图像适配器（合成验证）：** 新增 [`vs_seg_t1_adapter.py`](../vs_seg_t1_adapter.py)，只接受已通过 `qualify_vs_t1_request` 的固定模型请求与来源绑定的 MR `SeriesVolume`。输入限定为 DICOM 恒等 MR rescale：产品 `SeriesVolume` 对 MR 保留 stored values；KCL 的[官方说明](https://github.com/KCL-BMEIS/VS_Seg/blob/33410a2d44e3f57b4df1c3ed005e6d40c0824aa6/README.md)要求把 DICOM 经 3D Slicer 转为 NIfTI，[官方转换脚本](https://github.com/KCL-BMEIS/VS_Seg/blob/33410a2d44e3f57b4df1c3ed005e6d40c0824aa6/preprocessing/data_conversion.py)经 Slicer 导入并导出。ITK 的[GDCMImageIO 文档](https://docs.itk.org/projects/doxygen/en/latest/classitk_1_1GDCMImageIO.html)记录 RescaleSlope/Intercept，Kitware 说明该读入路径会应用这两个值；本适配器尚未对非恒等变换做可运行 Slicer 逐体素对照，因此明确拒绝这类序列。作者 test split header-only 检查为 46/46 系列、5680/5680 实例恒等变换。限定输入随后执行 `zyx → xyz`、LPS→RAS、MONAI 0.4 `Orientationd(RAS)` 轴置换/翻转及 `NormalizeIntensityd(nonzero=False)` 全体素均值/标准差；不重采样、不读取 label，并保留 request/source digest、rescale digest、来源 affine、方向变换和归一化统计。预测二值 mask 的逆变换重新核验同一来源绑定、shape 和值域，精确返回原始 `zyx` mask 与 LPS affine；它不加载权重、不运行模型、不将候选结果并入人工层。首轮官方对照发现实现只更新 affine 却漏翻像素数组，已修复并添加逐体素到 affine 的回归断言。修复后，作者 MONAI 0.4 Orientation+Normalize 对照中 axial LPS、反向 axial、coronal、sagittal 与 oblique 五类合成几何的预处理数组完全相同，affine 在 `1e-7` 内一致；模块 5 个测试覆盖像素映射、精确 mask 回环、zero-background 归一化、来源变更、非恒等/无效 MR rescale 拒绝和 fail-closed 值域，均通过。模型卡 `input_contract` 仅标记为上述恒等 rescale 子集的工程转换合同已验证；这不验证非恒等 Slicer/ITK 输入互通、真实病例序列自动判定或分割表现。详细输出留在 Git 忽略目录 `Annotation_Projects/vs-seg-t1-20260923/cpu-strict-load-20260923/synthetic-no-label-preprocess-result.json`，测试代码属于本仓库。

**2026-09-23 VS-Seg 隔离 worker 与结果协议（合成验证）：** 新增 [`vs_seg_t1_worker.py`](../vs_seg_t1_worker.py)。模块导入时不导入 Torch/MONAI；worker CLI 在独立 Python 进程运行，使用单线程 CPU，不依赖 Qt。父侧把已准备的 float32 `cxyz` 输入写入新建权限受限的 job 目录，以 `allow_pickle=False` 的 NPY 与严格字段 JSON 交换数据；清单只存来源绑定摘要，不写原始 Study/Series UID，并绑定请求摘要、恒等 MR rescale、RAS affine、输入字节/数组 SHA-256、固定 KCL 代码/权重版本、ROI 和输出类别。worker 启动前核对固定 KCL Python 文件 Git blob SHA-1 与 SHA-256、权重 ZIP SHA-256、checkpoint SHA-256；仅用 `weights_only=True` 安全读取并 `strict=True` 加载官方网络。兼容运行范围固定为已实测 Python 3.10、arm64、NumPy 1.26.4、Torch 2.5.1、MONAI 0.4.0；设备限定 CPU、单线程。分割路径按官方 `VSparams.py` 设置对 logits 主输出执行 ROI `384×384×64`、`sw_batch_size=1`、Gaussian blending、overlap 0.25 / sigma 0.125，最后按两通道 argmax 取前景通道 1；不阈值化、不分类、不自动采用。worker 返回 RAS `xyz` uint8 二值候选，父侧核验原请求未变、job/source/request/affine/模型摘要回显、输出文件/数组摘要、shape/dtype/值域，然后由既有 adapter 逆变换到同一源 `SeriesVolume` 的 `zyx`。该回映仅表示 IPC 与空间绑定检查通过；`validate_vs_result` 仍保留 `runtime_adapter_not_verified`，模型执行准入仍关闭。

验证：`tests/test_vs_seg_t1_worker.py` 9/9 合成测试通过，覆盖导入不拉起 Torch/MONAI、NPY/JSON staging、旧 job、篡改输入/输出、跨来源拒绝、argmax 和固定滑窗参数；合成 mask 从 RAS xyz 返回原源 zyx 逐体素一致。隔离探针 `Annotation_Projects/vs-seg-t1-20260923/cpu-strict-load-20260923/worker-contract-probe.py` 通过：固定资产重验及 `strict=True` 权重加载成功；MONAI 0.4 实际滑窗在 `5×6×4` 合成阵列和测试用 ROI `4×4×3` 上输出 shape 与 18 个前景体素位置一致；该探针的真实 KCL 网络前向调用数为 0，未读患者影像。`SKIP_REAL_DATA=1 QT_QPA_PLATFORM=offscreen /opt/miniconda3/envs/dicom_gui/bin/python -u tests/test_gui.py`：1495/1495 通过。Ruff、py_compile、`git diff --check` 通过。首轮统一测试曾把可选 worker runtime 的 lazy import 错判为桌面必装依赖；已将 Torch/MONAI 与 pinned KCL source 的限定理由写在 import 行并扩充依赖审计的窄豁免，随后完整回归全绿。

限制：尚未实现桌面侧进程 supervisor（启动/取消/超时/过期回调）、Qt/UI 接线或产品按钮；本轮没有通过 worker CLI 跑整例，也没有患者推理或指标。全病例 CPU 时间/峰值内存、46 例患者级 Dice/HD95/漏分与假阳性验证、阴性/OOD 和任务适配均未完成，不能据合成协议结果宣称分割效果。

**下一步：** 实现桌面侧隔离 worker supervisor，明确绑定指定 Python 环境，覆盖单任务、取消/超时、失败、旧结果与切换检查后的拒收；仅用 fake child/process 和合成任务验证，不开放 UI 运行按钮，不做患者前向。其后再单独评估整例 CPU 预算并决定患者 test split 验证是否可安全运行；执行准入继续关闭。
