...
> 当前开发版增补（2026-09-07）：本文已同步多序列标注、统一 Undo、`.miwproj` 保存恢复及三维配准操作。MRI 读取/编辑/配准目前通过合成数据验证，真实多序列 MRI 验收仍未完成；自动肿瘤定位、分割及类型预测仍在研究选型阶段。本文不代表已发布版本或整体功能验收完成，阶段证据见[实施计划](annotation_tumor_plan.md)。冻结的 V1.0 PDF 保持原样。
...
单击右侧面板顶部的「**加载 DICOM 目录**」按钮，选择包含切片的目录。当前读取入口接受 **Classic single-frame CT Image Storage / MR Image Storage**，拒绝 Enhanced、多帧及其他模态。同一检查的序列分别保存，可用检查/序列选择框切换；不同检查不会合成一个体积。MRI 的当前目标是静态结构序列；动态、扩散/多 echo 等输入尚未验证，不能据文件扩展名推断支持。对于可读取输入，软件将：
...
- **来源与空间分别校验**：可验证的来源帧身份和原始像素决定标注能否安全保存恢复；患者空间几何决定 MPR 和毫米测量能否使用。有效斜采集也可查看解剖三平面。几何不足但来源可靠时仅在原始切片上编辑，不生成伪毫米数据；来源身份也不足时禁用编辑。
- **CT 与 MR 分别门控**：MR 按 stored values 显示、使用 WW/WL 调灰度，不显示 HU，也不进入 CT 器官 AI、HU 定量、CT 随访或基于影像的 CT 重建。上述 CT 专用能力继续要求各自的完整几何/强度条件；内置模体仍可独立用于重建教学。
...
标准轴向来源的投影沿当前平面法向进行，**三个平面均支持**。斜采集目前仅支持单层查看与编辑，厚层投影禁用。选择「单层」时厚度框自动禁用，此时显示结果与不使用投影功能时**逐像素完全一致**。
...
标尺、自由线和 ROI 按创建时的位置保存；有患者空间时，在其他平面显示真实轮廓或交点。旧「全局穿透」仅是标准轴向来源切片上的重复二维参考，不代表三维病灶。普通标注、像素编辑、ROI 移动/缩放、删除和配准采用共用 `Ctrl+Z`，按实际操作顺序回退，最近 20 步随工程保存。
...
加载满足 CT 模型输入条件、且没有可恢复工作结果的序列后，软件在后台调用分割模型（`models/organs.onnx`，25 类胸腹器官含 5 个肺叶）进行整卷滑窗推理，右侧显示进度。MR 不进入该模型。工程中已保存的人工结果及有效空结果恢复后不自动重算。无模型文件或推理失败时，旧引擎可尝试连通域肺分割；它是受限的经典降级算法，不提供肿瘤类型。降级也失败时明确显示失败，不将其写成「未检出」。有效 AI 结果作为只读版本保存；新版本不覆盖已有人工工作结果，也不清除编辑历史。
...
先在图层下拉框中选择可编辑的工作结果；器官和病灶使用独立图层，可空间重叠。点击「新建病灶图层」可标注一个独立病灶，编号自动记录，未经过类型模型的结果保持未知。

同次检查的另一序列需要标注同一病灶时，先在参考序列选中该病灶工作图层，再切到目标序列并选择相应参考序列，点击「关联参考病灶」。该操作建立同编号的空图层，范围在目标序列独立绘制，按序列分别统计；关联本身可 Undo 并随工程保存。没有可靠空间对应、参考为器官或目标已有同编号图层时，入口禁用。

选择「分割画笔 / 分割橡皮」，把画笔半径降到最小的 `1 voxel`，单次点击只添加或删去一个来源体素；放大、平移或更换解剖平面不会把它变成一个屏幕像素大小的区域。连续按下、拖动至松开算一笔，`Ctrl+Z` 回退整笔；橡皮是删减工具，Undo 是回退操作，两者独立。切换工具、切片或序列会取消未完成的一笔。

选择「原始 AI」查看只读版本；「采用此 AI 版本为工作结果」是可撤销的操作。手工修订不会生成模型置信度或重新预测类型。「清空当前面」与「清空蒙版与标注」分开：后者清除当前工作图层的全部切片及本序列普通标注，确认框会说明范围；两者均可撤销。「重置」只恢复显示和布局，保留标注及历史。
...
两序列矩阵尺寸或行列像素间距不同时，或当前层超出既往序列的 z 覆盖范围时，软件标注「不可比」，停止配准和差值定量，并清空旧差值图。两幅图仍可并排查看；超出覆盖范围时，既往图明确标为最近端点，不能当作对应层面。
...
上述角度搜索仅适用于方形像素。两序列间距相同但像素非方形时，仅估计平移，标题显示「仅平移（非方形像素）」；像素索引中的旋转不能直接视作物理空间中的刚性旋转。
...
- **脱敏开关**：开启「脱敏（De-ID）」后，屏幕身份固定显示为 `ANON`，显式导出文件名使用本次加载生成的随机 `ANON-…` 别名并在重名时追加 suffix，因此连续导出不会静默覆盖。它不是 DICOM anonymizer：不会改写源 DICOM tags，不会移除 burned-in pixel text；内部 .miwproj 工程 / 旧 mask cache 仍保留用于匹配的 patient/series identifiers 与 geometry fingerprint，保存时界面会再次提示。
- **工程状态持久化**：AI-pending placeholder 不冒充已完成 AI 结果。工作图层被全部擦空或明确清空后，工程保留该空状态；重开不会让原始 AI 或旧缓存补回已删除内容。最近 20 步的 Undo 可跨次恢复。
...
「保存标注工程」保存当前检查中所有已接入序列、全部已标注切片及最近 20 步 Undo。默认目录是应用目录下的 `Annotation_Projects/`，可用「选择保存目录」更改，软件会记住选择。保存状态显示待保存、保存中、已保存时间或失败；悬停可看实际完整路径和格式，「打开保存目录」可定位文件。

每次完整操作后空闲 2 秒自动保存；连续操作时每 30 秒安排保存。自动结果和配准版本也会触发保存，不要求额外画一笔。正在拖画时只保存此前完整状态；保存途中继续编辑，界面会保持待保存，直到新版本落盘。关闭或切换检查会先处理最新保存，失败时可重试、换目录、留在当前工程或明确放弃修改。

工程文件扩展名为 `.miwproj`，它是单个 ZIP 包：`manifest.json` 记录来源、图层、时间和配准；每序列 NPZ 保存标签及可用置信度；`history.json` 与压缩差分保存 Undo；`summary.csv` 自动记录病灶编号、来源网格范围、体素数、类型/未知及修订状态。只有几何和单位可证明时才记录体积/HU，缺失项不填成 0。整个包一次提交，保存失败不会用半个新包替换上一完整文件。

### 12.2 打开、重新连接与旧格式

加载同一检查时优先恢复对应工程；也可用「打开工程」指定 `.miwproj`。工程不包含原始 DICOM，需要保留或重新提供原图。软件按 Study/Series/SOP、原始像素和空间绑定验证来源，不能仅凭同患者名或相同矩阵大小关联。

只载入某个序列时，其他序列的离线标注、结果、变换和历史仍保留，再次保存不会删掉它们。离线序列需加载包含其来源文件的目录后才能继续编辑；Undo 栈顶涉及离线来源时会停止并说明原因，不会跳过这一笔去撤销更早操作。

旧 `Exported_Lesions/` 中的 JSON/NPZ 仅兼容读取，迁移后保存至新工程。旧缓存只作为历史工作结果，无法补造当时的原始 AI、模型置信度或旧 Undo。损坏工程不会自动退回更早缓存；若仅历史损坏而最终状态完整，可明确选择创建不含坏历史的恢复副本，原件保留。最终图层/来源损坏时拒绝这种恢复方式。

### 12.3 同次 MRI 检查的对应查看

勾选「切换序列时定位同一点」，切换序列时尝试定位同一患者位置。共享 FrameOfReference 且空间重叠时只标作「元数据定位；未验证扫描间运动」。没有可靠对应或位置超出覆盖时不把点强夹到目标边缘。

选择参考序列，点击「三维刚性配准」计算候选；在弹出的 Axial / Coronal / Sagittal 三面窗口逐层比较当前、变换后参考和叠加图。「仅保留候选」不采用；明确点击「已检查三面，采用」后记录为人工复核，可通过统一 Undo 撤回。分数变好不等于真实对齐，人工复核也不等于标志点验证或临床验证。

「显示参考序列标注」以只读颜色显示对应 mask 和患者空间标注，不覆盖任一来源图层。二维全局参考无法跨序列投影。该功能的真实多序列 MRI 验收尚未完成；无几何、无重叠或配准失败时不能强套标注。
