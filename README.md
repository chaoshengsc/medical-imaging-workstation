...
| **Verification** | `v1.1.0` exact-SHA CI: **878 PASS / 0 FAIL**, **87% coverage**, Ruff PASS; 2026-08-30 local runs: **1013/0** full and **902/0** data-independent |
...
| **Status** | Annotation/MR development in progress; filing PDFs remain frozen and registration remains pending; no clinical or publication claim |
...
**An ablation that changed the product.** The engine was silently skipping nnU-Net's mandatory resampling to the training spacing. Across 20 ground-truth cases, Dice recovered **0.684 → 0.840**, improving **20 of 20** pairs (Wilcoxon *p* = 1.9×10⁻⁶). In a separate, unarchived cost measurement on the local RIDER series, inference fell from **100 s / 8.8 GB to 37 s / 3.0 GB**; these are different data conditions, not a paired accuracy–cost result on RIDER.
...
|---|---|
| **Clinical reading** | Classic single-frame CT/MR loading and per-Study series management · patient-space sorting and anatomical MPR, including valid oblique acquisitions · source-bound original-slice editing when spatial geometry is insufficient · independent gates for HU/CT organ AI, physical quantification and CT follow-up · canonical-series slab MIP / MinIP / AIP · measurement/ROI tools, PACS overlays and Cine. Enhanced/multi-frame and other modalities are rejected. MR real-case acceptance is pending |
...
| **Annotation projects (development)** | Independent organ/lesion working layers and read-only AI versions · one-source-voxel editing · unified latest-20 Undo across reopening · all retained series and slices in one `.miwproj` with automatic saving and JSON/NPZ/CSV records · same-Study metadata navigation and reviewed 3-D rigid correspondence. See the [current manual](docs/manual_en.md#12-annotation-project-persistence) and [acceptance record](docs/annotation_tumor_plan.md); this is not a claim of complete MRI or tumour-model validation |
...
| **Study III — learned sparse-view reconstruction** | A self-implemented 1.9 M-parameter residual U-Net reduces RMSE by **3–6×**, raises lesion-contrast retention from 0.87 to **0.957–0.996**, and retains an 0.81 out-of-distribution gain ratio. Across **60 noise-free synthetic paired phantoms**, the false-structure rate is **1.67%** at the 20%-of-lesion threshold and **0%** at 30% and 50%. | Photon noise was not applied. The 1.67% is neither an upper nor a lower bound for low-dose CT; both direction and magnitude under realistic photon noise are unmeasured, so low SNR is not claimed as the dominant driver. |
...
```bash
python tests/test_gui.py                     # 2026-08-30 local run: 1013 full-suite checks; local RIDER present
SKIP_REAL_DATA=1 python tests/test_gui.py    # 2026-08-30 local run: 902 data-independent checks
ruff check .                                 # lint
...
- **Display previews and HU analysis have separate requirements:** AI, HU measurements and follow-up require explicit `RescaleType=HU` or the classic CT `ORIGINAL` / non-`LOCALIZER` / non-multi-energy guarantee on every retained slice. When only the unit declaration is missing, a supported classic CT with a uniform positive finite slope/intercept can use all six window presets directly. This applies to the original local RIDER directory: no copied directory, tag editing or repeated confirmation is needed. The renderer applies the declared transform only to displayed slices and labels them as previews; the underlying unknown-unit array stays raw and HU-dependent analysis stays disabled. Explicit non-HU units, localizers, multi-energy data, unsupported LUTs and inconsistent transforms retain raw-value window sliders. The unit and transform distinction follows [DICOM C.8.2](https://dicom.nema.org/medical/dicom/current/output/chtml/part03/sect_C.8.2.html) and the [VOI display pipeline](https://dicom.nema.org/medical/dicom/current/output/chtml/part03/sect_C.11.2.html). The optional `tools/declare_rider_hu.py` still creates a separately declared copy; its histogram checks assess plausibility, not independent proof of units, and it is no longer required for window previews.
- **Series and mask state:** a successful series change clears the old HU probe and rebuilds the HUD in the new unit, while a failed load retains the old readout. AI-pending placeholders are not completed AI results. Fully erased or explicitly cleared working layers persist as empty in `.miwproj`, with the latest 20 Undo operations retained across reopening. Original AI and legacy caches do not refill manual results.
- **MR and tumour limits:** current MR annotation and 3-D correspondence have synthetic tests, with real multi-series MRI acceptance still pending. MR never enters CT-only HU or organ-model paths. No validated automatic tumour localisation/segmentation/type-prediction combination has been integrated; a manual lesion identifier is not a tumour diagnosis.
- **AI generalisation remains partly unmeasured:** 57 cases for lung lobes and 20 for the 21-organ aggregate are still small samples, all drawn from one public dataset (1.5 mm isotropic) **and all fed in the model's RAS in-plane convention rather than the product's DICOM (LPS) one**, so they measure the model rather than the shipped path; other protocols and scanners are untested, and per-organ reliability varies far more than the aggregate (liver 0.98 vs prostate 0.55). Spacing resampling is now in place (see the evidence table), but its finer-spacing side remains argued rather than measured, and it is skipped for very large scan ranges.
